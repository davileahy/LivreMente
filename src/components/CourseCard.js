...
import { Box, Image, Text, Tag, VStack, HStack, transition } from '@chakra-ui/react';
...
const CourseCard = ({ title, logo, description, categories, link }) => {
  return (
    ...
    >
      <HStack spacing={{ base: 4, md: 6 }} align="center" flexDirection={{ base: "column", md: "row" }}>
        ...
        <VStack align="start" spacing={3} w="100%">
          ...
          <HStack wrap="wrap" spacing={2} mt={2} justify={{ base: "center", md: "start" }}>
            {categories.map((category, index) => (
              <Tag key={index} colorScheme="teal" fontSize={{ base: "xs", md: "sm" }}>
                {category}
              </Tag>
            ))}
          </HStack>
